...
const Invoice: React.FC<InvoiceProps> = ({ order }) => {
  const downloadInvoice = async () => {
    ...
    const renderCustomerBox = (
      ...
      if (customer.name) doc.text(`Name : ${customer.name}`, x + 3, (lineY += 6));
      if (customer.phone)
    ...
    // Draw both sections
    const shippingBottomY = renderCustomerBox(leftX, currentY, "Shipping Details", order.customer);
    const billingBottomY = renderCustomerBox(rightX, currentY, "Billing Details", order.customer);
